...
module.exports = {
  ...
  help: function(req, res, next) {
    return res.view(theme + 'product/help.ejs')

    // res.sendfile(sails.config.appPath + '/assets/help.html')
  },
  ...
  list: function(req, res) {
    ...
    commonService.checkDbMongo(req, res).then(function(checkDbMongo) {
      console.log(
        'ProductController - list - check mongodb instance - checkDbMongo',
        checkDbMongo
      )
      ...
      if (checkDbMongo == 0 || typeof checkDbMongo == 'undefined') {
        ...
        var result = {
          user: req.session.hasOwnProperty('user')
            ? req.session.user
            : undefined
        }
        ...
        CoreReadDbService.getProductList().then(function(products) {
          ...
          var newIdProduct = CoreReadDbService.getCategoryList().then(function(
            categoryList
          ) {
